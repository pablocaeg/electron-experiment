...
const TrackedAccounts = [ '2210865044', '988608713215922176', '268687416', '876234971576446976', '1381428512872144901', '1032743790761705472', '2270781978', '1430475432239976451' ]
...
function deleteTweet() {
    ...
    if (del != "") {
        T.post('statuses/unretweet/:id', { id: del }, function(err) {
            if (err && console_mode == true) return logThis("ERROR: Se intentó borrar un tweet no publicado.\n")
...
function cross_follow() {
    ...
    T.get('followers/ids', {user_id: "1428150027558559751", count: 10, stringify_ids: true}, function(err, reply) {
...
function like() {
    T.get('friends/ids', {user_id: "1430475432239976451", count: 9, stringify_ids: true}, function(err, reply) {
        if (err && console_mode == true) return logThis("ERROR en: friends/ids\n");
        for (let i=0;i<9;i++) {
...
function curated_follow() {
    ...
    T.get('statuses/user_timeline', {user_id: "1430475432239976451", count: number_follow}, function(err, reply) {
        if (err && console_mode == true) return logThis("ERROR en: statuses/user_timeline\n")
        for (let i=0;i<3;i++) {
...
stream.on('tweet', function (tweet) {
    ...
    if (TrackedAccounts.indexOf(tweet.user.id_str) !== -1) {
        ...
        var StringToCheck2_pos = StringToCheck.indexOf('espiar')
        if (StringToCheck_pos ==  -1 && StringToCheck2_pos == -1  && tweet.user.id_str != "1430475432239976451" && tweet.in_reply_to_status_id == null) {
            if (tweet.retweeted == false) {
                T.post('statuses/retweet/:id', { id: tweet.id_str }, function(err) {
                    ...
                    T.get('statuses/user_timeline', {user_id: "1430475432239976451", count: 1, exclude_replies: true}, function(err, reply) {
...
stream.on('connect', function (item) { logThis('Stream connected.' + ' (' + 'twitter-frases.js' + ')' + '\n'); });
